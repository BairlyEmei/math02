...
# 读取数据
xls = pd.ExcelFile('第2周C题：接触式轮廓仪的自动标注(2020年D题)/附件4_工件2的局部测量数据（角）.xlsx')

# 读取所有sheet到字典
sheets = {sheet: xls.parse(sheet) for sheet in xls.sheet_names[:9]}
...
plt.tight_layout()
plt.savefig('Figures/Q4(2)_平移前的所有数据.png')
plt.show()
...
# plt.grid(True, linestyle='--', alpha=0.7)
# plt.tight_layout()
# plt.savefig('Figures/Q4(2)_最高点对齐后的所有数据.png')
# plt.show()

# 标记弧线区间
interval = [47, 51]
global_x_min = interval[0]
global_x_max = interval[1]
# 2. 创建参考x轴（在全局范围内均匀采样）
num_samples = 10000
...
result_df.to_excel('Figures/Q4(2)_平移并平均后的数据.xlsx', index=False)
...
for sheet_name, (x_data, translated_z) in translated_sheets.items():
    plt.plot(x_data, translated_z, label=sheet_name)
# 标记弧线区间
plt.plot(result_df['x'], result_df['z'], label='平均后', color='blue')
plt.axvspan(interval[0], interval[1], color='gray', alpha=0.3, label='弧线区间')
# 标记零点线（所有最低点将对齐到这条线）
plt.axhline(y=0, color='r', linestyle='--', alpha=0.7, label='零点（对齐线）')
plt.legend(loc='best')
plt.xlabel('X')
plt.ylabel('Z（平移后）')
plt.title('最高点对齐后的所有数据')
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('Figures/Q4(2)_最高点对齐后的所有数据.png')
plt.show()
