...
#区间
#[45.155,61.25]
interval=[47,48.2,48.8,49.6,49.86,50.55,50.85,51]

#读取数据
df=pd.read_excel("Figures/Q4(2)_平移并平均后的数据.xlsx")
...
for i, x_val in enumerate(interval):
    ...
    # 存储到字典中，a为x值，b为对应的z值
    turning_dict[f'a{i + 3}'] = df.loc[min_index, 'x']
    turning_dict[f'b{i + 3}'] = corresponding_z
...
for i in range(len(interval)):
    plt.scatter(turning_dict[f'a{i + 3}'], turning_dict[f'b{i + 3}'], s=30, color='black',zorder=10)
    plt.text(turning_dict[f'a{i + 3}'], turning_dict[f'b{i + 3}'] + 0.08, f'a{i + 3}', fontsize=20)

plt.title('工件2(角)的测量数据')
plt.xlabel('x')
plt.ylabel('z')
plt.grid(True, linestyle='--', alpha=0.7)
plt.tight_layout()
plt.savefig('Figures/Q4(2)_data.png')
plt.show()
plt.close()
...
line3=linear_fit(turning_dict['a4'],turning_dict['a5'],df,epsilon=0.2)
print('区间[a5,a6]的拟合结果：')
line4=linear_fit(turning_dict['a5'],turning_dict['a6'],df,epsilon=0.2)
...
print("转折点列表")
for i in range(len(interval)):
    print(f'(a{i + 3},b{i + 3})=({turning_dict[f'a{i + 3}']:.8f},{turning_dict[f'b{i + 3}']:.8f})')
...
def plot_all_results(df):
    ...
    plt.title('Q4(2)_data_fit', fontsize=24)
    ...
    # 6. 保存和显示图像
    plt.savefig('Figures/Q4(2)_data_fit.png', dpi=300, bbox_inches='tight')
    plt.show()
